...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(basedir, 'planetary.db')
...
def db_seed():
    ...
    test_user = User(first_name='Ogbonna',
                     last_name='Ekwueme',
                     email='georgekwm1@gmail.com',
                     password='p@ssw0rd')
...
def super_simple():
    return jsonify(message='Hello from the Planetry API'), 200
...
def parameters():
    ...
    if age < 18:
        return jsonify(message=f'Sorry {name}, you are not old enough.'), 200
    else:
...
def add_planet():
    ...
    if test:
        return jsonify(message=f'{planet_name} already exists'), 409
    else:
        planet_name = request.form['planet_name']
        ...
        db.session.commit()
        return jsonify(message='New planet created successfully'), 201
...
def update_planet():
    ...
    if planet:
        ...
        db.session.commit()
        return jsonify(message='Updated planet successfully'), 202
    else:
        return jsonify(message='Planet does not exist'), 404


@app.route('/remove_planet/<int:planet_id>', methods=['DELETE'])
def remove_planets(planet_id: int):
    planet = Planet.query.filter_by(planet_id=planet_id).first()
    if planet:
